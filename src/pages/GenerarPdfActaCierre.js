...
const GenerarPdfActaCierre = ({ isOpen, closeModal, codMesa }) => {
  ...
  const handleGetPDF = async (event) => {
    ...
    try {
      ...
      const response = await axios.get(url + `generarPDFActaCierreMesas/${codMesa}`);
    ...
    <Dialog open={modalPP} onClose={closeModalADDFP} fullWidth maxWidth="md">
      <DialogTitle>
        <Typography variant="h4" gutterBottom style={{ textAlign: 'center', marginBottom: '28px' }}>
          ACTA DE CIERRE - CODIGO MESA: {codMesa}
        </Typography>
      </DialogTitle>
...
export default GenerarPdfActaCierre;
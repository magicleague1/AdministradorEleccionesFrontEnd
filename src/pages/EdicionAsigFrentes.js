...
const AsignarFrente = ({ isOpen, closeModal }) => {
  ...
  const validarYActualizar = () => {
    if (selectedFrentes.length === 0) {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Frentes actualizados correctamente',
        text: 'Los Frentes se han actualizado con éxito!',
      }).then(() => {
        toogleCancelar();
      });;
    }
  ...
  return (
    ...
    >
      ...
      <br />
      <button className="custom-btn botonv1frente1" onClick={validarYActualizar}>
        Actualizar
      </button>
